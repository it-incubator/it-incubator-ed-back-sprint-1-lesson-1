...
describe('Driver API body validation check', () => {
  ...
  it('should not update driver when incorrect data passed; PUT /drivers/:id', async () => {
    ...
    const driverResponse = await request(app).get(`/drivers/${createdDriverId}`);
  ...
  it('should not update driver status when incorrect status passed; PUT /drivers/:id/status', async () => {
    const {
      body: { id: createdDriverId },
    } = await request(app)
      .post('/drivers')
    ...
    const driverBeforeUpdateResponse = await request(app).get(`/drivers/${createdDriverId}`);
    ...
    const driverAfterUpdateResponse = await request(app).get(`/drivers/${createdDriverId}`);
